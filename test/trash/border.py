...
if detected:
    ...
    for index, (resized_img, endpoint) in enumerate(zip(resized_images, label_endpoints)):
        ...
        extended_image[image_top_y:image_top_y + resized_img.shape[0], image_left_x:image_left_x + target_width] = resized_img

        # Rysowanie linii łączącej
        line_start = (endpoint[0], endpoint[1])
        line_end = (image_left_x + target_width // 2, current_y_position + resized_img.shape[0] // 2)

        cv2.line(extended_image, line_start, line_end, (0, 0, 255), 2)
